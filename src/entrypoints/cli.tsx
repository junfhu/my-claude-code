// =============================================================================
// cli.tsx — Bootstrap entrypoint for the Claude Code CLI
// =============================================================================
//
// This is the very first file that runs when a user invokes `claude` from their
// terminal.  Its primary design goal is **startup speed**: most CLI invocations
// only need a tiny fraction of the full codebase, so this file implements a
// series of "fast-path" dispatchers that dynamically import only the modules
// required for the requested operation and then exit early.
//
// The file is structured as follows:
//   1. Top-level side-effects (env vars that MUST be set before any imports)
//   2. An async main() containing a cascading series of fast-path checks
//   3. A final fallback that loads the full interactive CLI from main.tsx
//
// IMPORTANT: Every import inside main() is a dynamic `await import(...)` so
// that Node/Bun only evaluates the modules actually needed for that code path.
// =============================================================================

// `feature()` is a build-time macro provided by the bun bundler.  It returns a
// boolean constant that the bundler can evaluate at compile time, enabling
// dead-code elimination (DCE).  Code guarded by `feature('X')` where X is
// disabled in external builds will be entirely stripped from the output bundle.
import { feature } from 'bun:bundle';

// ── Top-level side-effects ───────────────────────────────────────────────────
// These env mutations MUST execute before any dynamic imports because several
// downstream modules capture env values into module-level constants at import
// time.  That means setting them later (e.g. inside init.ts) would be too late.

// Bugfix for corepack auto-pinning, which adds yarnpkg to peoples' package.jsons
// eslint-disable-next-line custom-rules/no-top-level-side-effects
process.env.COREPACK_ENABLE_AUTO_PIN = '0';

// Set max heap size for child processes in CCR environments (containers have 16GB)
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level, custom-rules/safe-env-boolean-check
if (process.env.CLAUDE_CODE_REMOTE === 'true') {
  // CCR (Claude Code Remote) containers have 16 GB of RAM.  We allocate half
  // (8 GB) to the V8 old-generation heap so that long-running sessions with
  // large context windows don't OOM while still leaving room for native memory,
  // child processes, and the OS.  The value is appended to any existing
  // NODE_OPTIONS so user-supplied flags aren't overwritten.
  // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
  const existing = process.env.NODE_OPTIONS || '';
  // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
  process.env.NODE_OPTIONS = existing ? `${existing} --max-old-space-size=8192` : '--max-old-space-size=8192';
}

// Harness-science L0 ablation baseline. Inlined here (not init.ts) because
// BashTool/AgentTool/PowerShellTool capture DISABLE_BACKGROUND_TASKS into
// module-level consts at import time — init() runs too late. feature() gate
// DCEs this entire block from external builds.
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.CLAUDE_CODE_ABLATION_BASELINE) {
  // When ablation baseline mode is active, force-disable every "smart" feature
  // so that benchmarks can measure the LLM's raw capability without the
  // enhancements Claude Code normally provides (thinking, compaction, auto-
  // memory, background tasks, etc.).  The ??= operator ensures that any
  // explicitly-set value takes precedence (i.e. only defaults are overridden).
  for (const k of ['CLAUDE_CODE_SIMPLE', 'CLAUDE_CODE_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'CLAUDE_CODE_DISABLE_AUTO_MEMORY', 'CLAUDE_CODE_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}

// ── Fast-path dispatcher ─────────────────────────────────────────────────────
// The main() function is the heart of this file.  It inspects process.argv to
// determine which "mode" the CLI was invoked in, then dynamically loads only
// the modules required for that mode.  Each branch is ordered roughly by
// frequency and how critical its startup latency is.

/**
 * Bootstrap entrypoint - checks for special flags before loading the full CLI.
 * All imports are dynamic to minimize module evaluation for fast paths.
 * Fast-path for --version has zero imports beyond this file.
 */
async function main(): Promise<void> {
  // Strip the first two argv entries (node/bun binary + script path) to get
  // the user-supplied arguments.
  const args = process.argv.slice(2);

  // ── --version / -v / -V ──────────────────────────────────────────────────
  // This is the fastest possible path: it prints a build-time constant and
  // returns without importing ANY modules.  This keeps `claude --version`
  // effectively instantaneous.
  // Fast-path for --version/-v: zero module loading needed
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    // MACRO.VERSION is inlined at build time
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(`${MACRO.VERSION} (Claude Code)`);
    return;
  }

  // ── Startup profiler ────────────────────────────────────────────────────────
  // Every path below --version benefits from startup timing data, so we load
  // the lightweight profiler here.  profileCheckpoint() records a high-res
  // timestamp that can later be printed or sent to analytics for diagnosing
  // slow startups.
  // For all other paths, load the startup profiler
  const {
    profileCheckpoint
  } = await import('../utils/startupProfiler.js');
  profileCheckpoint('cli_entry');

  // ── --dump-system-prompt (Anthropic-internal only) ──────────────────────────
  // Fast-path for --dump-system-prompt: output the rendered system prompt and exit.
  // Used by prompt sensitivity evals to extract the system prompt at a specific commit.
  // Ant-only: eliminated from external builds via feature flag.
  if (feature('DUMP_SYSTEM_PROMPT') && args[0] === '--dump-system-prompt') {
    profileCheckpoint('cli_dump_system_prompt_path');
    // enableConfigs() initializes the configuration system (settings files,
    // environment overrides) which the prompt renderer depends on.
    const {
      enableConfigs
    } = await import('../utils/config.js');
    enableConfigs();
    // Allow the caller to specify a model via `--model <name>`; otherwise
    // fall back to whatever model the main loop would normally use.
    const {
      getMainLoopModel
    } = await import('../utils/model/model.js');
    const modelIdx = args.indexOf('--model');
    const model = modelIdx !== -1 && args[modelIdx + 1] || getMainLoopModel();
    // Render the full system prompt (an array of sections) and print it.
    const {
      getSystemPrompt
    } = await import('../constants/prompts.js');
    const prompt = await getSystemPrompt([], model);
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(prompt.join('\n'));
    return;
  }
  // ── Chrome integration & computer-use MCP fast paths ───────────────────────
  // These three branches handle MCP (Model Context Protocol) server modes that
  // allow Claude to interact with Chrome or a desktop environment.  Each one
  // starts a long-running server process and never returns to the normal CLI.
  // Note: these use raw process.argv[2] instead of the `args` alias because
  // they were added before the convention of using the local `args` variable.

  // --claude-in-chrome-mcp: Starts an MCP server that exposes browser actions
  // (navigate, click, type, etc.) so Claude can control a Chrome tab.
  if (process.argv[2] === '--claude-in-chrome-mcp') {
    profileCheckpoint('cli_claude_in_chrome_mcp_path');
    const {
      runClaudeInChromeMcpServer
    } = await import('../utils/claudeInChrome/mcpServer.js');
    await runClaudeInChromeMcpServer();
    return;
  // --chrome-native-host: Implements Chrome Native Messaging protocol so the
  // Claude Code Chrome extension can communicate with the local CLI process
  // via stdin/stdout length-prefixed JSON messages.
  } else if (process.argv[2] === '--chrome-native-host') {
    profileCheckpoint('cli_chrome_native_host_path');
    const {
      runChromeNativeHost
    } = await import('../utils/claudeInChrome/chromeNativeHost.js');
    await runChromeNativeHost();
    return;
  // --computer-use-mcp (codename "Chicago"): Starts an MCP server that gives
  // Claude the ability to control a full desktop (mouse, keyboard, screenshots).
  // Gated behind the CHICAGO_MCP feature flag; DCE'd from public builds.
  } else if (feature('CHICAGO_MCP') && process.argv[2] === '--computer-use-mcp') {
    profileCheckpoint('cli_computer_use_mcp_path');
    const {
      runComputerUseMcpServer
    } = await import('../utils/computerUse/mcpServer.js');
    await runComputerUseMcpServer();
    return;
  }

  // ── --daemon-worker (internal, spawned by the daemon supervisor) ────────────
  // Fast-path for `--daemon-worker=<kind>` (internal — supervisor spawns this).
  // Must come before the daemon subcommand check: spawned per-worker, so
  // perf-sensitive. No enableConfigs(), no analytics sinks at this layer —
  // workers are lean. If a worker kind needs configs/auth (assistant will),
  // it calls them inside its run() fn.
  if (feature('DAEMON') && args[0] === '--daemon-worker') {
    // args[1] is the worker kind identifier (e.g. "assistant", "indexer")
    // passed by the supervisor when it fork/execs the child.
    const {
      runDaemonWorker
    } = await import('../daemon/workerRegistry.js');
    await runDaemonWorker(args[1]);
    return;
  }

  // ── remote-control / bridge mode ──────────────────────────────────────────
  // Fast-path for `claude remote-control` (also accepts legacy `claude remote` / `claude sync` / `claude bridge`):
  // serve local machine as bridge environment.
  // feature() must stay inline for build-time dead code elimination;
  // isBridgeEnabled() checks the runtime GrowthBook gate.
  if (feature('BRIDGE_MODE') && (args[0] === 'remote-control' || args[0] === 'rc' || args[0] === 'remote' || args[0] === 'sync' || args[0] === 'bridge')) {
    profileCheckpoint('cli_bridge_path');
    // Config must be loaded first because the bridge needs settings for auth,
    // API endpoints, and the GrowthBook feature-flag client.
    const {
      enableConfigs
    } = await import('../utils/config.js');
    enableConfigs();
    const {
      getBridgeDisabledReason,
      checkBridgeMinVersion
    } = await import('../bridge/bridgeEnabled.js');
    const {
      BRIDGE_LOGIN_ERROR
    } = await import('../bridge/types.js');
    const {
      bridgeMain
    } = await import('../bridge/bridgeMain.js');
    const {
      exitWithError
    } = await import('../utils/process.js');

    // Pre-flight validation: the bridge performs three sequential checks before
    // launching.  The order matters — auth must come first so that GrowthBook
    // can identify the user; version check ensures protocol compatibility;
    // policy check honours enterprise admin restrictions.

    // Auth check must come before the GrowthBook gate check — without auth,
    // GrowthBook has no user context and would return a stale/default false.
    // getBridgeDisabledReason awaits GB init, so the returned value is fresh
    // (not the stale disk cache), but init still needs auth headers to work.
    const {
      getClaudeAIOAuthTokens
    } = await import('../utils/auth.js');
    if (!getClaudeAIOAuthTokens()?.accessToken) {
      exitWithError(BRIDGE_LOGIN_ERROR);
    }
    // Check if the bridge feature is disabled server-side (e.g. kill-switch).
    const disabledReason = await getBridgeDisabledReason();
    if (disabledReason) {
      exitWithError(`Error: ${disabledReason}`);
    }
    // Ensure the local CLI version meets the minimum required by the bridge
    // protocol to prevent incompatible client/server pairs.
    const versionError = checkBridgeMinVersion();
    if (versionError) {
      exitWithError(versionError);
    }

    // Bridge is a remote control feature - check policy limits
    // Enterprise organizations can disable remote control via admin policies;
    // we must wait for the policy document to load before checking.
    const {
      waitForPolicyLimitsToLoad,
      isPolicyAllowed
    } = await import('../services/policyLimits/index.js');
    await waitForPolicyLimitsToLoad();
    if (!isPolicyAllowed('allow_remote_control')) {
      exitWithError("Error: Remote Control is disabled by your organization's policy.");
    }
    // All checks passed — hand off to the bridge's long-running event loop.
    // args.slice(1) strips the subcommand name, passing only sub-arguments.
    await bridgeMain(args.slice(1));
    return;
  }

  // ── daemon subcommand ──────────────────────────────────────────────────────
  // `claude daemon` starts (or manages) the long-running background supervisor
  // process that coordinates daemon workers.  Unlike --daemon-worker above,
  // the supervisor needs full configs and analytics sinks because it manages
  // lifecycle, scheduling, and health reporting.
  // Fast-path for `claude daemon [subcommand]`: long-running supervisor.
  if (feature('DAEMON') && args[0] === 'daemon') {
    profileCheckpoint('cli_daemon_path');
    const {
      enableConfigs
    } = await import('../utils/config.js');
    enableConfigs();
    // initSinks() wires up analytics / telemetry destinations so the daemon
    // can report health metrics and error events.
    const {
      initSinks
    } = await import('../utils/sinks.js');
    initSinks();
    const {
      daemonMain
    } = await import('../daemon/main.js');
    await daemonMain(args.slice(1));
    return;
  }

  // ── Background session management (ps / logs / attach / kill / --bg) ──────
  // Fast-path for `claude ps|logs|attach|kill` and `--bg`/`--background`.
  // Session management against the ~/.claude/sessions/ registry. Flag
  // literals are inlined so bg.js only loads when actually dispatching.
  if (feature('BG_SESSIONS') && (args[0] === 'ps' || args[0] === 'logs' || args[0] === 'attach' || args[0] === 'kill' || args.includes('--bg') || args.includes('--background'))) {
    profileCheckpoint('cli_bg_path');
    const {
      enableConfigs
    } = await import('../utils/config.js');
    enableConfigs();
    // Import all bg handlers at once — they share module-level state so a
    // single import is cheaper than cherry-picking.
    const bg = await import('../cli/bg.js');
    // Route to the appropriate handler based on the subcommand.  If none of
    // the named subcommands match (i.e. the user passed `--bg` alongside a
    // normal prompt), fall through to handleBgFlag which spawns a background
    // session for the current command.
    switch (args[0]) {
      case 'ps':
        await bg.psHandler(args.slice(1));
        break;
      case 'logs':
        await bg.logsHandler(args[1]);
        break;
      case 'attach':
        await bg.attachHandler(args[1]);
        break;
      case 'kill':
        await bg.killHandler(args[1]);
        break;
      default:
        // --bg / --background was passed with a normal command — detach into
        // a background session.
        await bg.handleBgFlag(args);
    }
    return;
  }

  // ── Template job commands (new / list / reply) ─────────────────────────────
  // Templates let users create, list, and respond to pre-defined prompt
  // templates via `claude new`, `claude list`, `claude reply`.  These render
  // an Ink-based TUI (React for the terminal) which can leave dangling event-
  // loop handles, so we force-exit with process.exit(0) instead of returning.
  // Fast-path for template job commands.
  if (feature('TEMPLATES') && (args[0] === 'new' || args[0] === 'list' || args[0] === 'reply')) {
    profileCheckpoint('cli_templates_path');
    const {
      templatesMain
    } = await import('../cli/handlers/templateJobs.js');
    await templatesMain(args);
    // process.exit (not return) — mountFleetView's Ink TUI can leave event
    // loop handles that prevent natural exit.
    // eslint-disable-next-line custom-rules/no-process-exit
    process.exit(0);
  }

  // ── environment-runner (BYOC — Bring Your Own Compute) ────────────────────
  // A headless runner mode where the user's own infrastructure polls for and
  // executes Claude Code tasks.  This is a long-running process that never
  // shows a TUI.
  // Fast-path for `claude environment-runner`: headless BYOC runner.
  // feature() must stay inline for build-time dead code elimination.
  if (feature('BYOC_ENVIRONMENT_RUNNER') && args[0] === 'environment-runner') {
    profileCheckpoint('cli_environment_runner_path');
    const {
      environmentRunnerMain
    } = await import('../environment-runner/main.js');
    await environmentRunnerMain(args.slice(1));
    return;
  }

  // ── self-hosted-runner ────────────────────────────────────────────────────
  // Similar to environment-runner but targets the SelfHostedRunnerWorkerService
  // API.  The runner registers itself, then enters a poll loop where each poll
  // doubles as a heartbeat (no separate keepalive needed).
  // Fast-path for `claude self-hosted-runner`: headless self-hosted-runner
  // targeting the SelfHostedRunnerWorkerService API (register + poll; poll IS
  // heartbeat). feature() must stay inline for build-time dead code elimination.
  if (feature('SELF_HOSTED_RUNNER') && args[0] === 'self-hosted-runner') {
    profileCheckpoint('cli_self_hosted_runner_path');
    const {
      selfHostedRunnerMain
    } = await import('../self-hosted-runner/main.js');
    await selfHostedRunnerMain(args.slice(1));
    return;
  }

  // ── --worktree + --tmux combo ──────────────────────────────────────────────
  // When both --worktree and --tmux are present, the CLI creates a fresh git
  // worktree for the session and exec()'s itself inside a new tmux pane.  This
  // must happen BEFORE loading the full CLI because exec() replaces the current
  // process entirely.
  // Fast-path for --worktree --tmux: exec into tmux before loading full CLI
  const hasTmuxFlag = args.includes('--tmux') || args.includes('--tmux=classic');
  if (hasTmuxFlag && (args.includes('-w') || args.includes('--worktree') || args.some(a => a.startsWith('--worktree=')))) {
    profileCheckpoint('cli_tmux_worktree_fast_path');
    const {
      enableConfigs
    } = await import('../utils/config.js');
    enableConfigs();
    // The worktree feature can be disabled via config; check before attempting.
    const {
      isWorktreeModeEnabled
    } = await import('../utils/worktreeModeEnabled.js');
    if (isWorktreeModeEnabled()) {
      const {
        execIntoTmuxWorktree
      } = await import('../utils/worktree.js');
      // execIntoTmuxWorktree either replaces this process (handled=true) or
      // returns with an error if something went wrong (e.g. tmux not installed).
      const result = await execIntoTmuxWorktree(args);
      if (result.handled) {
        return;
      }
      // If not handled (e.g., error), fall through to normal CLI
      if (result.error) {
        const {
          exitWithError
        } = await import('../utils/process.js');
        exitWithError(result.error);
      }
    }
    // If worktree mode is disabled, fall through to load the normal CLI which
    // will handle the flags in its own way (or ignore them).
  }

  // ── --update / --upgrade redirect ─────────────────────────────────────────
  // Users commonly type `claude --update` or `claude --upgrade` expecting an
  // update.  Rather than erroring, silently rewrite argv to the proper
  // `claude update` subcommand so the full CLI picks it up naturally.
  // Redirect common update flag mistakes to the update subcommand
  if (args.length === 1 && (args[0] === '--update' || args[0] === '--upgrade')) {
    process.argv = [process.argv[0]!, process.argv[1]!, 'update'];
  }

  // ── --bare mode ───────────────────────────────────────────────────────────
  // --bare: set SIMPLE early so gates fire during module eval / commander
  // option building (not just inside the action handler).
  // Setting CLAUDE_CODE_SIMPLE=1 this early ensures that feature gates
  // evaluated at module load time (before commander parses options) see
  // the simplified mode.  Without this, some modules would initialize in
  // "full" mode and never switch.
  if (args.includes('--bare')) {
    process.env.CLAUDE_CODE_SIMPLE = '1';
  }

  // ── Default path: load the full interactive CLI ───────────────────────────
  // If no fast-path matched, we fall through to here and load the complete
  // CLI framework (commander, Ink TUI, tool registry, etc.) from main.tsx.
  // This is the heaviest path in terms of module loading.

  // Start buffering stdin keypresses immediately so that anything the user
  // types while we're still loading modules isn't lost.
  // No special flags detected, load and run the full CLI
  const {
    startCapturingEarlyInput
  } = await import('../utils/earlyInput.js');
  startCapturingEarlyInput();
  // Record timing around the heavy main.tsx import — this is the single
  // largest chunk of module evaluation in the entire CLI.
  profileCheckpoint('cli_before_main_import');
  const {
    main: cliMain
  } = await import('../main.js');
  profileCheckpoint('cli_after_main_import');
  // Hand control to the full CLI.  This never returns during normal
  // interactive usage (Ink's render loop keeps the process alive).
  await cliMain();
  profileCheckpoint('cli_after_main_complete');
}

// ── Kick off the entrypoint ──────────────────────────────────────────────────
// `void main()` invokes main() and intentionally discards the returned Promise.
// Any unhandled rejection will be caught by Node's global unhandledRejection
// handler rather than being silently swallowed.
// eslint-disable-next-line custom-rules/no-top-level-side-effects
void main();
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
