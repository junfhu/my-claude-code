...
// =============================================================================
// This module handles conversation history compression ("compaction") to keep
// the conversation within the model's context window limits.
//
// How compaction works:
//   1. When the conversation approaches the context limit (auto-compact) or
//      the user explicitly requests it, this module summarizes the older
//      messages into a concise summary while preserving recent history.
//
//   2. The summary is generated by a "forked subagent" — a separate API call
//      that receives the full conversation history and produces a condensed
//      version. This uses runForkedAgent() which creates an isolated API
//      request with its own system prompt optimized for summarization.
//
//   3. After compaction, the conversation is replaced with:
//      - A compact boundary marker (SystemCompactBoundaryMessage) that
//        indicates where the compaction occurred in the message history
//      - The summary messages (condensed version of older conversation)
//      - Recent messages that were preserved (not summarized)
//      - Re-injected attachments (CLAUDE.md, skill instructions, etc.)
//      - Hook results from pre/post-compact hooks
//
// Auto-compact triggers:
//   - Token count exceeds the model's context window threshold
//   - A prompt-too-long error is returned by the API (reactive compaction)
//
// Key patterns:
//   - Images are stripped before summarization (saves tokens, not needed for summary)
//   - Skill discovery/listing attachments are stripped (re-injected post-compact)
//   - PTL (prompt-too-long) retries progressively drop oldest message groups
//   - File contents read during the conversation are re-read post-compact
//     (up to POST_COMPACT_MAX_FILES_TO_RESTORE files within token budget)
// =============================================================================
...
// Feature-gated import: session transcript tracking (KAIROS feature flag)
...
// markPostCompaction signals that the conversation has been compacted,
// which affects cache behavior and system prompt construction
...
// FileReadTool is used post-compact to re-read important files back into context
...
// Message types — the compact service operates on the full Message union type
// and produces SystemCompactBoundaryMessage markers to delimit compacted regions.
import type {
  AssistantMessage,
  AttachmentMessage,
  HookResultMessage,
...
// Attachment utilities — used to re-inject CLAUDE.md, skill listings, MCP
// instructions, and other context attachments after compaction.
import {
...
// Maximum output tokens the compaction summary is allowed to generate
import { COMPACT_MAX_OUTPUT_TOKENS } from '../../utils/context.js'
// Context analysis — measures token usage breakdown for analytics
import {
  analyzeContext,
...
// runForkedAgent creates an isolated API call with its own system prompt —
// this is the "forked subagent" pattern used to generate the compaction summary.
// CacheSafeParams carries prompt-cache-safe parameters across the fork.
import {
  type CacheSafeParams,
  runForkedAgent,
} from '../../utils/forkedAgent.js'
// Pre/post-compact hooks allow extensions (IDE integrations, custom workflows)
// to inject instructions or perform side effects around compaction events.
import {
...
// Message utilities — createCompactBoundaryMessage produces the marker that
// separates pre-compact history from post-compact summary in the message list.
// getMessagesAfterCompactBoundary extracts only the messages after the last
// compaction point (used when re-compacting an already-compacted conversation).
import {
...
// =============================================================================
// Post-Compaction Constants
// =============================================================================
// After compaction, the system re-injects critical context (recently-read files,
// active skill instructions) to avoid losing important working state. These
// constants govern the token budget and limits for that re-injection.

// Maximum number of files to re-read from disk and inject after compaction.
// Files are selected by recency (most recently read first).
export const POST_COMPACT_MAX_FILES_TO_RESTORE = 5
// Total token budget for all re-injected file contents combined.
export const POST_COMPACT_TOKEN_BUDGET = 50_000
// Per-file token cap — prevents a single large file from consuming the entire budget.
...
// Per-skill token cap — truncates individual skill instructions to this limit.
export const POST_COMPACT_MAX_TOKENS_PER_SKILL = 5_000
// Total token budget for all re-injected skill instructions combined.
export const POST_COMPACT_SKILLS_TOKEN_BUDGET = 25_000
// Maximum number of times to retry the compaction API call when it itself
// hits prompt-too-long. Each retry drops the oldest message groups.
 ...
 */
// =============================================================================
// stripImagesFromMessages — Removes media blocks before compaction
// =============================================================================
// Images and documents are replaced with text markers ("[image]", "[document]")
// so the summarizer knows media was shared without consuming vision tokens.
// This also prevents the compaction API call itself from hitting prompt-too-long.
export function stripImagesFromMessages(messages: Message[]): Message[] {
  return messages.map(message => {
    // Only user messages can contain images (direct attachments or tool_result content)
    if (message.type !== 'user') {
      return message
    }

    // Skip non-array content (simple string messages have no media blocks)
    const content = message.message.content
    if (!Array.isArray(content)) {
      return message
    }

    // Track whether any media blocks were found to avoid unnecessary cloning
    let hasMediaBlock = false
    // Replace image/document blocks with text markers, including those
    ...
    // If no media blocks were found, return the original message unmodified
    // to preserve referential equality and avoid unnecessary object creation.
    if (!hasMediaBlock) {
      return message
    }
 ...
 */
// =============================================================================
// stripReinjectedAttachments — Removes attachments that will be re-added post-compact
// =============================================================================
// Skill discovery and skill listing attachments are re-surfaced automatically
// after compaction (via resetSentSkillNames + the next turn's discovery signal).
// Feeding them to the summarizer wastes tokens and pollutes the summary.
...
// Sentinel error message used when there's nothing to compact
export const ERROR_MESSAGE_NOT_ENOUGH_MESSAGES =
  'Not enough messages to compact.'
// Maximum number of prompt-too-long retry attempts for the compaction call itself
const MAX_PTL_RETRIES = 3
// Synthetic marker prepended after truncation so the summarizer knows context was lost
const PTL_RETRY_MARKER = '[earlier conversation truncated for compaction retry]'

// =============================================================================
// truncateHeadForPTLRetry — Drops oldest messages when compaction itself is too long
// =============================================================================
...
// =============================================================================
// User-facing error messages for compaction failures
// =============================================================================
// These are displayed to the user when compaction fails for various reasons.
...
// =============================================================================
// CompactionResult — The output of a successful compaction operation
// =============================================================================
// Contains everything needed to replace the conversation with its compacted
// form: the boundary marker, summary, preserved messages, and re-injected
// attachments. Also includes token count metrics for analytics.
export interface CompactionResult {
  // The system message marking where compaction occurred in history
  boundaryMarker: SystemMessage
  // The condensed summary of older conversation history
  summaryMessages: UserMessage[]
  // Re-injected context attachments (CLAUDE.md, skills, MCP instructions)
  attachments: AttachmentMessage[]
  // Results from post-compact hooks (IDE integrations, custom extensions)
  hookResults: HookResultMessage[]
  // Recent messages preserved verbatim (not summarized)
  messagesToKeep?: Message[]
  // Optional user-facing message about the compaction
  userDisplayMessage?: string
  // Token count before compaction (for analytics)
  preCompactTokenCount?: number
  // Token count after compaction (for analytics)
  postCompactTokenCount?: number
  // True token count after all re-injections (may differ from postCompactTokenCount)
  truePostCompactTokenCount?: number
  // Detailed token usage from the compaction API call itself
  compactionUsage?: ReturnType<typeof getTokenUsage>
}

// =============================================================================
// RecompactionInfo — Diagnostic metadata for compaction analytics
// =============================================================================
// Tracks whether this is a re-compaction (compacting an already-compacted
// conversation) and how many turns have passed since the last compaction.
// This helps diagnose compaction loops where the system keeps re-compacting.
...
// =============================================================================
// Enforces a consistent ordering for all compaction paths:
//   1. boundaryMarker — the SystemCompactBoundaryMessage separator
//   2. summaryMessages — the condensed conversation summary
//   3. messagesToKeep — recent messages preserved verbatim
//   4. attachments — re-injected CLAUDE.md, skills, MCP instructions
//   5. hookResults — output from post-compact hooks
...
// =============================================================================
// compactConversation — The main compaction entry point
// =============================================================================
// Orchestrates the full compaction workflow:
//   1. Run pre-compact hooks (allows extensions to inject custom instructions)
//   2. Snapshot pre-compact token count for analytics
//   3. Strip images and re-injectable attachments from messages
//   4. Send the cleaned messages to the forked summarizer subagent
//   5. Handle prompt-too-long retries (progressively drop oldest groups)
//   6. Construct the CompactionResult with boundary marker, summary, and attachments
//   7. Run post-compact hooks
//   8. Re-inject critical context (CLAUDE.md, skill instructions, etc.)
//
// The function is called from two paths:
//   - Auto-compact: triggered when token count exceeds threshold
//   - Manual compact: triggered by the user via /compact command
