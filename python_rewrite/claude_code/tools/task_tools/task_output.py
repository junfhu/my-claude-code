"""TaskOutputTool – Retrieve a task's output."""

from __future__ import annotations

from typing import Any, Callable, Optional

from pydantic import BaseModel, Field

from claude_code.tool import (
    PermissionBehavior,
    PermissionDecision,
    Tool,
    ToolProgress,
    ToolResult,
    ToolUseContext,
    ValidationResult,
)
from claude_code.tools.utils import format_tool_error, truncate_output, MAX_OUTPUT_CHARS


class TaskOutputInput(BaseModel):
    task_id: str = Field(..., description="The task ID.")


class TaskOutputTool(Tool):
    """Get the output of a completed or running task."""

    name = "task_output"
    aliases: list[str] = []
    search_hint = "task output result"

    def get_input_schema(self) -> dict[str, Any]:
        return TaskOutputInput.model_json_schema()

    async def get_description(self, input: dict[str, Any]) -> str:
        return "Get the output of a task."

    async def validate_input(
        self, input: dict[str, Any], context: ToolUseContext
    ) -> ValidationResult:
        if not input.get("task_id"):
            return ValidationResult(result=False, message="task_id is required.")
        return ValidationResult(result=True)

    async def check_permissions(
        self, input: dict[str, Any], context: ToolUseContext
    ) -> PermissionDecision:
        return PermissionDecision(behavior=PermissionBehavior.ALLOW, updated_input=input)

    async def call(
        self,
        args: dict[str, Any],
        context: ToolUseContext,
        on_progress: Optional[Callable[[ToolProgress], None]] = None,
    ) -> ToolResult:
        parsed = TaskOutputInput.model_validate(args)
        tasks = context.extra.get("tasks", {})
        task = tasks.get(parsed.task_id)
        if not task:
            return ToolResult(data=format_tool_error(f"Task not found: {parsed.task_id}"))

        output = task.get("output")
        if output is None:
            return ToolResult(data=f"Task {parsed.task_id} has no output yet (status: {task['status']}).")

        return ToolResult(data=truncate_output(str(output), MAX_OUTPUT_CHARS))

    def is_read_only(self, input: dict[str, Any]) -> bool:
        return True

    def user_facing_name(self, input: Optional[dict[str, Any]] = None) -> str:
        return "TaskOutput"
